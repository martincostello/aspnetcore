...
public partial class WebApplicationBuilderAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        if (!IsConfigureInvocation(wellKnownTypes, methodSymbol))
        {
            return;
        }
    ...
    private static bool IsConfigureInvocation(
        WellKnownTypes wellKnownTypes,
        IMethodSymbol targetMethod)
    {
        return targetMethod is not null &&
            targetMethod.Name.Equals("Configure", StringComparison.Ordinal) &&
